...
$pdf->AddFont('ArialNarrow','B','arialnb.php');
            ...
            if(isset($_POST['educ_lads'])){
                $educ_id = $_POST['educ_lads'];                
					...
					$title_educ_query = $db->query("SELECT * from chem_educator_tbl WHERE educ_id='$educ_id'");
				...
				$lads_query= $db->query("SELECT * FROM chem_sched_tbl WHERE sched_educ_id = '$educ_id'");
                ...
                while($lads_row = $lads_query->fetch()){
                    ...
                    $educ_query = $db->query("SELECT * from chem_educator_tbl WHERE educ_id='$lads_educ_id'");
                    $educ_array = $educ_query->fetch();

                    $course_query = $db->query("SELECT * from chem_course_tbl WHERE course_id='$lads_course_id'");
                    $course_array = $course_query->fetch();

                    $room_query = $db->query("SELECT * from chem_room_tbl WHERE room_id='$lads_room_id'");
					...
					$pdf->Cell(165);
					$pdf->Write(5,'TIPM - CPL - 004');
					$pdf->Ln(3);
					$pdf->SetFont('ArialNarrow','',8);
					$pdf->Cell(158);
					$pdf->Write(5,'REVISION STATUS/DATE: 2/2017 APRIL 20');
					$pdf->Ln(10);
            ...
            /*
				$queryEvent = 'SELECT * FROM chem_event_tbl ORDER BY day_id ASC, start ASC, room_id ASC';
				...
				while($rowEvent =mysqli_fetch_array($resultEvent)){
					...
					$rect_counter +=1;
						...
						$instEvent= "SELECT instructor_name FROM chem_instructor_tbl WHERE instructor_id='$instructorID'";
						...
						$queryRM= "SELECT room_num FROM chem_room_tbl WHERE room_id='$roomID'";
