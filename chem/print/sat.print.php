...
function Header()
{
	...
	$this->Cell(212);
	$this->Write(5,'REVISION STATUS/DATE: 2/2023 JUNE 26');
	$this->Ln(10);
    // Arial bold 15
...
$pdf->SetWidths(array(104.9,95,30,25,15));
            ...
            while($time_row=$time_query->fetch()){
				...
				/*$sched_query= "SELECT * FROM chem_sched_tbl WHERE sched_day_id = 1 AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC";
				$sched_result = pg_query($db_handle, $sched_query);*/
                
                $sched_query = $db->query("SELECT * FROM chem_sched_tbl WHERE sched_day_id='6' AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC");
				
						/*$sched_count= "SELECT * FROM chem_sched_tbl WHERE sched_day_id = 1  AND sched_start='$time_start'";
						$count_result = pg_query($db_handle, $sched_count);
						$sched_count_total = pg_num_rows($count_result);*/

                        $sched_count= $db->query("SELECT * FROM chem_sched_tbl WHERE sched_day_id = 6  AND sched_start='$time_start'");
				...
				while($sched_row = $sched_query->fetch()){
                    ...
                    $room_query = "SELECT * from chem_room_tbl WHERE room_id='$sched_room_id'";
                    ...
                    $room_query = $db->query("SELECT * from chem_room_tbl WHERE room_id='$sched_room_id'");
            ...
            $pdf->Cell(60,5,'Angelica C. Bareno',0,0,'L');
	        $pdf->Cell(110,5,'Emmylou Dimaculangan',0,0,'L');
            ...
            $pdf->Ln(10);
            $pdf->Cell(60,5,'James Nel S. Palma',0,0,'L');
            $pdf->Ln(4);
            $pdf->Cell(60,5,'Laboratory Technician I',0,0,'L');
