...
$pdf->SetWidths(array(95,50,18.3,18.3,18.3,30,25,15));
            ...
            while($time_row=$time_query->fetch()){
                ...
                $sched_query = $db->query("SELECT * FROM etr_sched_tbl WHERE sched_day_id='6' AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC");
                        ...
                        $sched_count= $db->query("SELECT * FROM etr_sched_tbl WHERE sched_day_id = 6  AND sched_start='$time_start'");
            ...
            $pdf->Cell(60,5,'Ralph Randall M. Dominguez',0,0,'L');
	        $pdf->Cell(110,5,'Ma. Marjorie Z. Lorenzo',0,0,'L');
            $pdf->Cell(80,5,'Engr. Norman Giovanni M. Guevarra',0,0,'L');
            $pdf->Ln(4);
            $pdf->Cell(60,5,'Laboratory Technician II',0,0,'L');
            $pdf->Cell(110,5,'Laboratory Technician III',0,0,'L');
            $pdf->Cell(80,5,'Laboratories and Shops Coordinator',0,0,'L');
            $pdf->Ln(10);
            $pdf->Cell(60,5,'Delmer Carl M. Salirungan, RME',0,0,'L');
            $pdf->Cell(110,5,'Jerald B. Cultivo',0,0,'L');
            $pdf->Ln(4);
