      ...
      <?php
          if (isset($_GET['page_no']) && $_GET['page_no']!="") {
            $page_no = $_GET['page_no'];
              ...
              /*$result_count = pg_query($db_handle, "SELECT COUNT(*) As total_records FROM etr_educator_tbl ");
              $total_records = pg_fetch_array($result_count);
              $total_records = $total_records['total_records'];
              $total_no_of_pages = ceil($total_records / $total_records_per_page);
              $second_last = $total_no_of_pages - 1; // total page minus 1*/

              $result_count = $db->query("SELECT COUNT(*) As total_records FROM etr_educator_tbl");
              $total_records = $result_count->fetch();
              $total_records = $total_records['total_records'];
              $total_no_of_pages = ceil($total_records / $total_records_per_page);
              $second_last = $total_no_of_pages - 1; // total page minus 1

          /*$educ_query = "SELECT * from etr_educator_tbl ORDER BY educ_lname ASC LIMIT $total_records_per_page OFFSET $offset";
          $educ_result = pg_query($db_handle, $educ_query);*/

          $educ_query = $db->query("SELECT * from etr_educator_tbl ORDER BY educ_lname ASC LIMIT $total_records_per_page OFFSET $offset");
