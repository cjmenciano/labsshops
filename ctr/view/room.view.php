      ...
      <?php
          if (isset($_GET['page_no']) && $_GET['page_no']!="") {
            $page_no = $_GET['page_no'];
              ...
              $result_count = $db->query("SELECT COUNT(*) As total_records FROM ctr_room_tbl");
              $total_records = $result_count->fetch();
              $total_records = $total_records['total_records'];
              $total_no_of_pages = ceil($total_records / $total_records_per_page);
              $second_last = $total_no_of_pages - 1; // total page minus 1

          /*$room_query = "SELECT * from ctr_room_tbl ORDER BY room_num ASC LIMIT $total_records_per_page OFFSET $offset";
          $room_result = pg_query($db_handle, $room_query);*/
          $room_query = $db->query("SELECT * from ctr_room_tbl ORDER BY room_num ASC LIMIT $total_records_per_page OFFSET $offset");
