      ...
      <?php
          if (isset($_GET['page_no']) && $_GET['page_no']!="") {
            $page_no = $_GET['page_no'];
              ...
              $result_count = $db->query("SELECT COUNT(*) As total_records FROM ctr_course_tbl");
              $total_records = $result_count->fetch();
              $total_records = $total_records['total_records'];
              $total_no_of_pages = ceil($total_records / $total_records_per_page);
              $second_last = $total_no_of_pages - 1; // total page minus 1

          /*$course_query = "SELECT * from ctr_course_tbl ORDER BY course_code ASC LIMIT $total_records_per_page OFFSET $offset";
          $course_result = pg_query($db_handle, $course_query);*/
          $course_query = $db->query("SELECT * from ctr_course_tbl ORDER BY course_code ASC LIMIT $total_records_per_page OFFSET $offset");
