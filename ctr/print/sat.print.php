...
$pdf->SetWidths(array(95,50,18.3,18.3,18.3,30,25,15));
            ...
            while($time_row=$time_query->fetch()){
				...
				/*$sched_query= "SELECT * FROM ctr_sched_tbl WHERE sched_day_id = 1 AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC";
				$sched_result = pg_query($db_handle, $sched_query);*/
                
                $sched_query = $db->query("SELECT * FROM ctr_sched_tbl WHERE sched_day_id='6' AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC");
				
						/*$sched_count= "SELECT * FROM ctr_sched_tbl WHERE sched_day_id = 1  AND sched_start='$time_start'";
						$count_result = pg_query($db_handle, $sched_count);
						$sched_count_total = pg_num_rows($count_result);*/

                        $sched_count= $db->query("SELECT * FROM ctr_sched_tbl WHERE sched_day_id = 6  AND sched_start='$time_start'");
				...
				while($sched_row = $sched_query->fetch()){
                    ...
                    /*$educ_query = "SELECT * from ctr_educator_tbl WHERE educ_id='$sched_educ_id'";
                    $educ_result = pg_query($db_handle, $educ_query);
                    $educ_array = pg_fetch_array($educ_result);
        
                    $course_query = "SELECT * from ctr_course_tbl WHERE course_id='$sched_course_id'";
                    $course_result = pg_query($db_handle, $course_query);
                    $course_array = pg_fetch_array($course_result);
        
                    $room_query = "SELECT * from ctr_room_tbl WHERE room_id='$sched_room_id'";
                    ...
                    $educ_query = $db->query("SELECT * from ctr_educator_tbl WHERE educ_id='$sched_educ_id'");
                    $educ_array = $educ_query->fetch();

                    $course_query = $db->query("SELECT * from ctr_course_tbl WHERE course_id='$sched_course_id'");
                    $course_array = $course_query->fetch();

                    $room_query = $db->query("SELECT * from ctr_room_tbl WHERE room_id='$sched_room_id'");
            ...
            $pdf->Cell(60,5,'Allan S. Tenedero',0,0,'L');
            $pdf->Cell(110,5,'Mark Christian R. Musa',0,0,'L');
            $pdf->Cell(80,5,'Engr. Norman Giovanni M. Guevarra',0,0,'L');
            $pdf->Ln(4);
            $pdf->Cell(60,5,'Sr. Laboratory Technician',0,0,'L');
            $pdf->Cell(110,5,'Sr. Laboratory Technician',0,0,'L');
            $pdf->Cell(80,5,'Laboratories and Shops Coordinator',0,0,'L');
            $pdf->Ln(10);
            $pdf->Cell(60,5,'Jason S. Legaspi',0,0,'L');
            $pdf->Cell(110,5,'Clifford John R. Menciano',0,0,'L');
            $pdf->Ln(4);
            $pdf->Cell(60,5,'Laboratory Technician III',0,0,'L');
            $pdf->Cell(60,5,'Laboratory Technician I',0,0,'L');
            $pdf->Ln(10);
            $pdf->Cell(60,5,'Jose Alfonso A. Zacarias',0,0,'L');
            $pdf->Ln(4);
