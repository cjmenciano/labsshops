...
$pdf->SetWidths(array(95,50,18.3,18.3,18.3,30,25,15));
            ...
            while($time_row=$time_query->fetch()){
                ...
                $sched_query = $db->query("SELECT * FROM ctr_sched_tbl WHERE sched_day_id='1' AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC");
                        ...
                        $sched_count= $db->query("SELECT * FROM ctr_sched_tbl WHERE sched_day_id = 1  AND sched_start='$time_start'");
				...
				while($sched_row = $sched_query->fetch()){
                    ...
                    $pdf->Row(array($course_array['course_code'].'-'.$sched_row['sched_section']." ".$course_array['course_desc'], $educ_array['educ_fname'].' '.$educ_array['educ_lname'],'','','',date("g:i A", strtotime($sched_row['sched_start'])).' -'.date("g:i A", strtotime($sched_row['sched_end'])),$day_array['days'],$room_array['room_num']));
					}						
			}
