	...
	function Header()
		{
			...
			$this->Cell(200,5,'LABORATORIES AND SHOPS DEPARTMENT',0,0,'C');
			$this->Ln(7);
...
$pdf->AddFont('ArialNarrow','B','arialnb.php');
            ...
            if(isset($_POST['educ_lads'])){
                $educ_id = $_POST['educ_lads'];                
					...
					$title_educ_query = $db->query("SELECT * from ctr_educator_tbl WHERE educ_id='$educ_id'");
				...
				$lads_query= $db->query("SELECT * FROM ctr_sched_tbl WHERE sched_educ_id = '$educ_id'");
                ...
                while($lads_row = $lads_query->fetch()){
                    ...
                    $educ_query = $db->query("SELECT * from ctr_educator_tbl WHERE educ_id='$lads_educ_id'");
                    $educ_array = $educ_query->fetch();

                    $course_query = $db->query("SELECT * from ctr_course_tbl WHERE course_id='$lads_course_id'");
                    $course_array = $course_query->fetch();

                    $room_query = $db->query("SELECT * from ctr_room_tbl WHERE room_id='$lads_room_id'");
					...
					$pdf->Cell(158);
					$pdf->Write(5,'REVISION STATUS/DATE: 2/2017 NOV 28');
					$pdf->Ln(10);
					$pdf->SetFont('Arial','B',18);
					$pdf->Cell(200,5,'LABORATORIES AND SHOPS DEPARTMENT',0,0,'C');
					$pdf->Ln(7);
					$pdf->Cell(200,5,'LABORATORY ACTIVITY DATA SHEET',0,0,'C');
					$pdf->Ln(5);
					...
					$pdf->Cell(100,5,$educ_array['educ_dept'],0,0,'L');
					$pdf->Ln(5);
					$pdf->SetFont('Arial','',12);
					$pdf->Cell(200,5,'(PROGRAM)',0,0,'C');
					$pdf->Ln(10);
            ...
            /*
				$queryEvent = 'SELECT * FROM ctr_event_tbl ORDER BY day_id ASC, start ASC, room_id ASC';
				...
				while($rowEvent =mysqli_fetch_array($resultEvent)){
					...
					$rect_counter +=1;
						...
						$instEvent= "SELECT instructor_name FROM ctr_instructor_tbl WHERE instructor_id='$instructorID'";
						...
						$queryRM= "SELECT room_num FROM ctr_room_tbl WHERE room_id='$roomID'";
