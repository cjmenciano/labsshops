...
function Header()
{
	...
	$this->Cell(220);
	$this->Write(5,'TIPM - CPL - 005');
	$this->Ln(3);
	$this->SetFont('ArialNarrow','',8);
	$this->Cell(212);
	$this->Write(5,'REVISION STATUS/DATE: 1/2017 APR 20');
	$this->Ln(10);
    // Arial bold 15
    ...
    $this->Cell(33);
    // Title
    $this->Write(5,'CHEMISTRY & PHYSICS LABORATORIES DEPARTMENT');
    // Line break
    $this->Ln(8);
	...
	$this->Ln(10);
	$this->SetFont('ArialNarrow','B',12);
	$this->Cell(104.9,10,'COURSE CODE/ DESCRIPTION',1,0,'C');
	$this->Cell(95,10,'FACULTY',1,0,'C');
	//$this->Cell(54.9,5,'LECTURE SCHEDULE',1,0,'C');
	$this->Cell(70,5,'LAB SCHEDULE',1,0,'C');
	$this->Ln(5);
	$this->Cell(104.9,5,'',0,0,'C');
...
$pdf->SetWidths(array(104.9,95,30,25,15));

            $time_query = $db->query("SELECT time from time_tbl");
            while($time_row=$time_query->fetch()){
				$time_start =$time_row['time'];
				
				/*$sched_query= "SELECT * FROM phys_sched_tbl WHERE sched_day_id = 1 AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC";
				$sched_result = pg_query($db_handle, $sched_query);*/
                
                $sched_query = $db->query("SELECT * FROM phys_sched_tbl WHERE sched_day_id='6' AND sched_start='$time_start' ORDER BY sched_day_id ASC, sched_start ASC, sched_room_id ASC");
				
						/*$sched_count= "SELECT * FROM phys_sched_tbl WHERE sched_day_id = 1  AND sched_start='$time_start'";
						$count_result = pg_query($db_handle, $sched_count);
						$sched_count_total = pg_num_rows($count_result);*/

                        $sched_count= $db->query("SELECT * FROM phys_sched_tbl WHERE sched_day_id = 6  AND sched_start='$time_start'");
				...
				while($sched_row = $sched_query->fetch()){
                    ...
                    /*$educ_query = "SELECT * from phys_educator_tbl WHERE educ_id='$sched_educ_id'";
                    $educ_result = pg_query($db_handle, $educ_query);
                    $educ_array = pg_fetch_array($educ_result);
        
                    $course_query = "SELECT * from phys_course_tbl WHERE course_id='$sched_course_id'";
                    $course_result = pg_query($db_handle, $course_query);
                    $course_array = pg_fetch_array($course_result);
        
                    $room_query = "SELECT * from phys_room_tbl WHERE room_id='$sched_room_id'";
                    ...
                    $educ_query = $db->query("SELECT * from phys_educator_tbl WHERE educ_id='$sched_educ_id'");
                    $educ_array = $educ_query->fetch();

                    $course_query = $db->query("SELECT * from phys_course_tbl WHERE course_id='$sched_course_id'");
                    $course_array = $course_query->fetch();

                    $room_query = $db->query("SELECT * from phys_room_tbl WHERE room_id='$sched_room_id'");
            ...
            $pdf->Cell(80,5,'Noted by:',0,0,'L');
            $pdf->Ln(10);
            $pdf->SetFont('ArialNarrow','',12);
            $pdf->Cell(60,5,'Denmark R. Villa',0,0,'L');
	        $pdf->Cell(110,5,'Princess Bhernadette M. Roaquin',0,0,'L');
            $pdf->Cell(80,5,'Engr. Lorraine A. Carillo',0,0,'L');
            $pdf->Ln(4);
            $pdf->Cell(60,5,'Laboratory Technician I',0,0,'L');
            $pdf->Cell(110,5,'Laboratory Technician I',0,0,'L');
            $pdf->Cell(80,5,'Head, Chemistry and Physics Laboratory',0,0,'L');
$pdf->Output('I','Saturday-Laboratory Courses Master Schedule.pdf');
?>